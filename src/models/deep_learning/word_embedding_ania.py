...
"""
WordEmbeddingANIA model for MIC prediction in the AMP-MIC project.

This module defines the WordEmbeddingANIA model, which processes word embedding features using an Inception-based architecture,
...
class BasicConv1d(nn.Module):
    """
    A basic Conv1D block with BatchNorm and ReLU.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of BasicConv1d block.
        """
        return self.relu(self.bn(self.conv(x)))


class WordEmbeddingInceptionModule(nn.Module):
    """
    Inception Module for word embedding features in the AMP-MIC project.

    This module processes word embedding features using a multi-branch Inception structure,
    ...
    def __init__(self, in_channels: int = 1024, out_channels: int = 128):
        """
        Initialize the Word Embedding Inception Module.

        Parameters
        ----------
        in_channels : int
            Number of input channels (default is 1024 for word embedding features).
        out_channels : int
        ...
        # Branch 3: 1x1 Conv -> 5x1 Conv (adapted from Inception_v1's 5x5)
        self.branch5x5_1 = BasicConv1d(
            in_channels,
            branch_channels // 2,
            kernel_size=1,
            stride=1,
        ...
        # Branch 4: Pooling -> 1x1 Conv (adapted from Inception_v1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of the Word Embedding Inception Module.

        Parameters
        ----------
        x : torch.Tensor
            Input tensor of shape (batch_size, in_channels, max_len).

        Returns
        -------
        torch.Tensor
            Output tensor of shape (batch_size, out_channels, max_len).
        """
        ...
        branch5x5 = branch5x5 * self.branch_weights[2]
        branch_pool = branch_pool * self.branch_weights[3]

        out = [branch1x1, branch3x3, branch5x5, branch_pool]
        out = torch.cat(out, 1)
...
class WordEmbeddingANIA(nn.Module):
    """
    WordEmbeddingANIA (Word Embedding-specific part of An Inception-Attention Network for Predicting the Minimum Inhibitory Concentration)
    for MIC prediction in the AMP-MIC project.

    This model processes word embedding features through an Inception Module, followed by
    Multi-Head Attention and Dense layers to predict MIC values.
    """
    ...
    def __init__(
        ...
        """
        Initialize the WordEmbeddingANIA model.
        ...
        # Inception Module
        self.inception = WordEmbeddingInceptionModule(
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass of the WordEmbeddingANIA model.
        ...
        """
        x = self.inception(x)  # (batch_size, inception_out_channels, max_len)

        # Reshape to sequence: (batch_size, max_len, channels)
        batch_size, channels, max_len = x.size()
        x = x.view(batch_size, channels, max_len).permute(
            0, 2, 1
        )  # (batch_size, max_len, channels)

        # Project to d_model
        x = self.projection(x)  # (batch_size, max_len, d_model)

        # Multi-Head Attention
        attn_output, attn_weights = self.attention(
            x, x, x, need_weights=True
        )  # (batch_size, max_len, d_model), (batch_size, num_heads, max_len, max_len)
        if self.training:
            self.attn_weights = attn_weights

        # Global average pooling over sequence dimension
        x = attn_output.mean(dim=1)  # (batch_size, d_model)
    ...
    def compute_gradcam(self, branch_name: str) -> torch.Tensor:
        ...
        -------
        torch.Tensor
            Grad-CAM heatmap of shape (max_len), where max_len is determined by the feature map.
        """
        ...
        # Compute global average of gradients over sequence dimension
        grad_weights = gradients.mean(dim=2, keepdim=True)  # (batch_size, channels, 1)
        ...
        return gradcam[0]  # (max_len) for the first sample
